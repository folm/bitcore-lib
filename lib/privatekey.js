...
PrivateKey._transformBuffer = function(buf, network) {
  ...
  if(!info.network || (network && info.network !== Networks.get(network)))  {
    info.network = Networks.get(buf[0], 'electrumprivkey');
    buf = new Buffer.concat([
      new Buffer([0xb2]),
      buf.slice(1, 33), 
      (buf.length === 1 + 32 + 1 && buf[1 + 32 + 1 - 1] === 1) ? new Buffer([1]) : undefined
    ]);
  }
